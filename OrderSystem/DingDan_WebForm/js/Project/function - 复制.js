...
layui.use(['form', 'util', 'laytpl', 'laydate', 'layer'], function () {
    ...
    $(document).ajaxStart(function () {
       ...
       layer.alert('程序出现错误,请重试或联系管理员!',{icon:2})
       return false;
    });
            ...
            $.ajax({
                ...
                success:function(data){
                    if (data.flag == "1") {
                        count = 60;
                        $.each($trs, function (i, v) {
                            if ($(v).find(".code").text() == data.dt[i].code) {
                        ...
                        layer.msg("库存更新成功!", { icon: 1, time: 1000 })
                    } else {
                        count=10
        ...
        $trs.each(function (i, v) {
            ...
            if ($(v).find(".fAvailQtty").text() == 0 || $(v).find(".realqty").text() == 0) {
                ...
                if (($(v).find(".num").text() - $(v).find(".fAvailQtty").text()) > 0 || ($(v).find(".num").text() - $(v).find(".realqty").text()) > 0) {
                    console.log($(v).find(".num").text() - $(v).find(".fAvailQtty").text());
                    console.log($(v).find(".num").text() - $(v).find(".realqty").text())
                    $(v).addClass("yellow");
                }
            }
        })
...
form.on('select(txtArea)',function(data){
    console.log($("#buy_list tbody tr").length)
  if ($("#buy_list tbody tr").length==0||data.value == 0 || data.value == ""|| data.value==undefined ||$("#TxtCustomer option:selected").val()==""||$("input[name=shfs]:checked").val()=='配送') {
            return false;
        }
        var tds = $("#buy_list tbody tr").find(".code"),
            c = [],
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.list_dt.length > 0) {
                    // if (data.list_dt[0][0].cbdefine16!=100) {
                    //     $("#is_discount").removeClass('layui-hide');
                    // }else   {
                    //       $("#is_discount").addClass('layui-hide');
                    // }
                    $.each($("#buy_list tbody tr"), function (i, v) {
                        if ($(v).find(".code").text() == data.list_dt[i][0].cInvCode) {
                            $(v).find(".fAvailQtty").text(data.list_dt[i][0].fAvailQtty);
        ...
        layer.open({
            ...
            btn1: function () {
                if ($("#Remarks").val().trim().length>200) {
                    layer.alert('备注不能超过200个字',{icon:2})
                    return false;
                }
...
function get_html_num(data) {
    ...
    $.each(data, function (i, v) {
        ...
        html += "<td class='realqty'>" + v.iquantity + "</td>";
        html += "<td class='fAvailQtty'>" + (v.Stock == null ? 0 : v.Stock) + "</td>";
        html += "<td class='pack'>" + (typeof (v.pack) == 'undefined' ? "" : v.pack) + "</td>";
...
function get_formData() {
    ...
    formData.txtArea = $txtArea.text() == "自提必须选择行政区" ? "" : $txtArea.text();
     ...
     $.ajax({
           ...
           async:false,
            ...
            success:function(res){
               ...
               var html="<option value>请选择自提(托运)地址</option>";
               $.each(res.DataSet.address_dt,function(i,v){
                html+='<option strDistrict="'+v.strDistrict+'" value="'+v.lngopUseraddressId+'">'+v.strDistributionType+','+v.strConsigneeName+','+v.strConsigneeTel+','+v.strDistrict+v.strReceivingAddress+'</option>'
               })
