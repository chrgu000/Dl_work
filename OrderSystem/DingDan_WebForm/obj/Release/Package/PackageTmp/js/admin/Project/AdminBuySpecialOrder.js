    ...
    if (!flag) {
    	...
    	return false;
    		...
    		HeadData.ccusname=$('#TxtCustomer option:selected').text();
    	 	HeadData.cMemo=$("#strRemarks").val().trim();
    	  	$.each(tb_options.data,function(i,v){
    	 	...
    	 	HeadData.BodyData=tb_options.data;
    	 	  ...
    	 	  $.ajax({
                ...
                dataType: "Json",
                data: {
                    "Action": "DLproc_NewYOrderByIns_Admin",
                ...
                success: function (res) {
                	...
                	layer.alert("您的订单已提交成功,订单号为:<br />" + res.code,{icon:1,closeBtn:0},function(){
...
$(document).on('click','#del',function(){
	 ...
	 if (ck.data.length>0) {
	 	layer.confirm('确定要删除勾选项？',{icon:3},function(){
	 		if (ck.isAll) {
	 			tb_options.data=[];
	 		}else{
	 			 var c=[];
