...
$(document).on("click", "#Dao", function() {
    ...
    var xlf = document.getElementById('uploadExcel');
     ...
     var html = '<div style="width:80%;margin:10px auto"><form class="layui-form">\
    <a href="javascript:;" class="file layui-btn">选择Excel<input type="file" name="" id="uploadExcel"></a>\
    <div class="layui-input-inline"><select id="codeconfig_select_excel" lay-search><option value=1>添加凑整</option><option value=2>减少凑整</option><option value=3>重置为0</option></select></div>\
    <a class="layui-btn layui-btn-mini" href="/Tpl/ExcelTpl.xls">下载模板</a><button type="button" class="layui-btn layui-btn-mini">查看帮助</button></form>\
   <hr/> <div id="uploadExcel_msg_div"></div></div>';
     layer.open({
         type: 1,
         title: '导入Excel',
         area: ['650px', '600px'],
...
$(function() {
	...
	$(document).on('change', '#uploadExcel', function(event) {
		...
		reader.onload = function(e) {
			...
			if ( ExcelData['Sheet1'].length==0||!('自定义编码' in ExcelData['Sheet1'][0])) {
				h += '<p>请将第一列列名设置为:自定义编码。 </p>';
			}
			// if (!('num_s' in o)) {
			...
			if (EmptyCode != '' || ErrNum != '' || ReCode != '') {
				...
				if (EmptyCode != '') {
					html += '<blockquote class="layui-elem-quote" style="border-left:5px solid #ff8c8c" ><div style="text-align: center;font-size:16px;font-weight:bold">有编码为空的行</div>' + EmptyCode + '</blockquote>';
				}
				if (ErrNum != '') {
				...
				if (ReCode != '') {
					html += '<blockquote class="layui-elem-quote" style="border-left:5px solid #ff8c8c"><div style="text-align: center;font-size:16px;font-weight:bold">有编码重复的行</div>' + ReCode + '</blockquote>';
				}
				$('#uploadExcel_msg_div').html(html);
			...
			var cWhCode=$("#txtWareHouse_div").find("option:selected").val()==undefined?"CD01":$("#txtWareHouse_div").find("option:selected").val();
			$.ajax({
				traditional: true,
				type: "Post",
				...
				success: function(res) {
					...
					if (res.table.length > 0) {
						...
						//循环添加产品数量
						var codeconfig_select = $('#codeconfig_select_excel').val();
					...
					var html = '<blockquote class="layui-elem-quote" style="border-left:5px solid #79C48C" ><div style="text-align: left;">成功导入' + res.ja.length + '个编码</div></blockquote>';
					if (res.Errcodes && res.Errcodes.length > 0) {
						var h = '';
						$.each(res.Errcodes, function(i, v) {
							h += '<p><strong>' + v + '</strong></p>';
						})
						html += '<blockquote class="layui-elem-quote" style="border-left:5px solid #ff8c8c" ><div style="text-align: left;">有以下共计' + res.Errcodes.length + '个编码没有查到，请核实:</div>' + h + '</blockquote>';
					...
					if (res.LimitCodes && res.LimitCodes.length > 0) {
						...
						html += '<blockquote class="layui-elem-quote" style="border-left:5px solid #ff8c8c" ><div style="text-align: left">有以下共计' + res.LimitCodes.length + '个编码被限销，请核实:</div>' + h + '</blockquote>';
					}
					//layer.alert(html);
						$('#uploadExcel_msg_div').html(html);
