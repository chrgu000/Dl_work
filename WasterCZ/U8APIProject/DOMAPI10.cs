...
namespace U8APIProject
{
    ...
    public class DOMAPI10
    {
        ...
        public APIRet Add(Object u8Login, API10Hd hd, List<API10Bd> bds)
        {
            ...
            ENUtil.SetToDomH(domhead, "ddate", DateTime.Now.ToString("yyyy-MM-dd"));  ///20181130 余长城修改，用于月底中班单据入到下个月
            ...
            catch (Exception e)
            {
                broker.Release();
                apiRet.Success = false;
                apiRet.ErrMsg = "产成品入库单添加单据API内部异常：" + e.Message;
                apiRet.VouchId = "";
                return apiRet;
            }
        ...
        public String Audit(Object u8Login, String vouchId)
        {
            ...
            {
                broker.Release();
                return "产成品入库单审核单据API内部异常：" + e.Message;
            }
