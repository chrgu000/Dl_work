...
namespace DAL
{
    public class SearchDAO
    {
        ...
        #region 获取客户对应的自定义代码[DL_CustomerCodeBySel]
        /// <summary>
        /// 获取客户对应的自定义代码[DL_CustomerCodeBySel]
        /// </summary>
        /// <param name="phone">手机号码</param>
        /// <param name="pwd">密码</param>
        /// <returns></returns>
        public DataTable DL_CustomerCodeBySel(string phone, string pwd)
        {
            DataTable dt = new DataTable();
            string cmdText = "select a.cCusInvCode,a.cInvName,a.cInvStd,a.UnitGroup,a.cComUnitName,b.cInvDefine2,b.cInvDefine1,b.cInvDefine14,b.cInvDefine13 FROM dbo.Dl_opInvCodeConfig a LEFT JOIN dbo.Dl_opInventoryForPacket b ON a.cInvCode=b.cInvCode WHERE strAllAcount=(SELECT aa.cCusCode FROM dbo.Dl_opUser aa INNER JOIN dbo.Customer bb ON aa.ccuscode=bb.cCusCode WHERE cCusPhone LIKE '%" + @phone + "%' AND aa.strUserPwd='" + @pwd + "' UNION ALL SELECT strAllAccount FROM dbo.Dl_opUser_Ex WHERE strSubPhone='" + @phone + "' AND strSubPwd='" + @pwd + "')";
        ...
        /// <param name="cCusCode">顾客编码</param>
        /// <returns></returns>
        public DataTable DLproc_TreeListDetailsAllBySel(string cInvCCode, string cCusCode, int iShowType)
        {
            DataTable dt = new DataTable();
        ...
        #region 获取物料树明细_特殊订单(全部,包含大类)[DLproc_TreeListDetailsAll_TSBySel]
        /// <summary>
        /// 获取物料树明细
        /// </summary>
        /// <param name="cInvCCode">物料大类编码</param>
        /// <param name="cCusCode">顾客编码</param>
        /// <returns></returns>
        public DataTable DLproc_TreeListDetailsAll_TSBySel(string cInvCCode, string cCusCode)
        {
            DataTable dt = new DataTable();
            string cmdText = "DLproc_TreeListDetailsAll_TSBySel";
            SqlParameter[] paras = new SqlParameter[] { 
           new SqlParameter("@cInvCCode",cInvCCode),
           new SqlParameter("@cCusCode",cCusCode)           
           };
        ...
        #region 获取物料(预订单参照生成酬宾,特殊订单)树明细(特殊订单)[DLproc_TreeListPreDetails_TSBySel]
        /// <summary>
        /// 获取物料(预订单参照生成酬宾,特殊订单)树明细(特殊订单)[DLproc_TreeListPreDetails_TSBySel]
        /// </summary>
        /// <param name="strBillNo">预订单编号</param>
        ///  <param name="lngBillType">单据类型,1酬宾,2特殊</param>
        /// <returns></returns>
        public DataTable DLproc_TreeListPreDetails_TSBySel(string strBillNo, int lngBillType)
        {
            DataTable dt = new DataTable();
        ...
        #region 用于顾客查询客户的欠款分析（对账单）,所有的客户信息[DL_U8SOASearchOfOperBySel]
        /// <summary>
        ///  用于顾客查询客户的欠款分析（对账单）,所有的客户信息[DL_U8SOASearchOfOperBySel]
        /// </summary>
        /// <param name="ComboPeriodYear_exp">年</param>
        /// <param name="ComboPeriodMon_exp">月</param>
        /// <returns></returns>
        public DataTable DL_U8SOASearchOfOperBySel(string ComboPeriodYear_exp, string ComboPeriodMon_exp)
        {
            DataTable dt = new DataTable();
            string cmdText = "SELECT * FROM dbo.Dl_opU8SOA WHERE intPeriodYear=@ComboPeriodYear_exp AND intPeriod=@ComboPeriodMon_exp ORDER BY ccuscode";
            SqlParameter[] paras = new SqlParameter[] { 
            new SqlParameter("@ComboPeriodYear_exp",ComboPeriodYear_exp),
            new SqlParameter("@ComboPeriodMon_exp",ComboPeriodMon_exp)         
           };
            dt = sqlhelper.ExecuteQuery(cmdText, paras, CommandType.Text);
            return dt;
        }
        ...
        /// <param name="user">操作帐号</param>
        /// <returns></returns>
        public DataTable DLproc_OrderExecuteBySel(string strBillNo, string ccuscode, string begindate, string enddate, string showtype, string FHStatus, string user)
        {
            DataTable dt = new DataTable();
  ...
  new SqlParameter("@strAllAcount",user)
           };
            dt = sqlhelper.ExecuteQuery(cmdText, paras, CommandType.StoredProcedure);
            return dt;
        }
        ...
        /// <param name="FHStatus">包含未发货(1)</param>
        /// <returns></returns>
        public DataTable DLproc_OrderExecuteFordatAuditordTimeBySel(string strBillNo, string ccuscode, string begindate, string enddate, string showtype, string FHStatus, string user)
        {
            DataTable dt = new DataTable();
        ...
        #region 查询该客户的所有账户[DL_AllCountBySel]
        /// <summary>
        /// 查询该客户的所有账户[DL_AllCountBySel]
        /// </summary>
        /// <param name="strUser">Session["ConstcCusCode"]登录帐号</param>
        /// <returns></returns>
        public DataTable DL_AllCountBySel(string strUser)
        {
            DataTable dt = new DataTable();
            string cmdText = "SELECT strAllAccount,strAllAccount cCusCode  FROM dbo.Dl_opUser_Ex WHERE strLoginName=@strUser UNION ALL select @strUser,@strUser";
            SqlParameter[] paras = new SqlParameter[] { 
        ...
        #region 参照特殊订单审核(审核关闭)[DL_CZTSCloseByUpd]
        /// <summary>
        /// 参照特殊订单审核(审核关闭)[DL_CZTSCloseByUpd]
        /// </summary>
        /// <param name="n"></param>
        ...
        {
            bool flag = false;
            string cmdText = "  UPDATE dbo.Dl_opOrder SET bytStatus=96 , InvalidPersonCode=strManagers,InvalidTime=getdate() WHERE strBillNo=@strBillNo";
            SqlParameter[] paras = new SqlParameter[] { 
             new SqlParameter("@strBillNo",BillNo)       
            };
            int res = sqlhelper.ExecuteNonQuery(cmdText, paras, CommandType.Text);
        ...
        #region 参照特殊订单审核(审核生成U8发货单)[DLproc_NewOrderToDispU8ByIns]
        /// <summary>
        /// 参照特殊订单审核(审核生成U8发货单)[DLproc_NewOrderToDispU8ByIns]
        /// </summary>
        /// <param name="n"></param>
        /// <returns>BillNo</returns>
        public bool DLproc_NewOrderToDispU8ByIns(string BillNo)
        {
            bool flag = false;
            string cmdText = "DLproc_NewOrderToDispU8ByIns";
            SqlParameter[] paras = new SqlParameter[] { 
             new SqlParameter("@strBillNo",BillNo)       
            };
        ...
        #region 参照特殊订单审核(审核前先检测该订单关联的预定的销售订单是否已经通过审核)[DL_CZTSCheckBySel]
        /// <summary>
        /// 参照特殊订单审核(审核前先检测该订单关联的预定的销售订单是否已经通过审核)[DL_CZTSCheckBySel]
        /// </summary>
        /// <param name="n"></param>
        /// <returns>BillNo</returns>
        public DataTable DL_CZTSCheckBySel(string BillNo)
        {
            DataTable dt = new DataTable();
            string cmdText = "SELECT DISTINCT aa.cSOCode   FROM dbo.SO_SOMain aa LEFT JOIN dbo.SO_SODetails bb ON aa.ID=bb.ID WHERE bb.cpreordercode IN ( SELECT DISTINCT bb.cpreordercode FROM dbo.Dl_opOrder aa LEFT JOIN dbo.Dl_opOrderDetail bb ON aa.lngopOrderId=bb.lngopOrderId WHERE aa.strBillNo=@strBillNo )  and dverifysystime IS null";
            SqlParameter[] paras = new SqlParameter[] { 
             new SqlParameter("@strBillNo",BillNo)       
            };
            dt = sqlhelper.ExecuteQuery(cmdText, paras, CommandType.Text);
