...
namespace DAL
{
    public class LoginDAO
    {
        ...
        #region 检查用户登录是否成功(Login_wx)
        /// <summary>
        /// 检查用户登录是否成功(Login_wx)
        /// </summary>
        /// <param name="username">用户名</param>
        /// <returns>登录是否成功</returns>
        public DataTable Login_wx(string username)
        {
            DataTable dt = null;

            string cmdText = "select top(1) aa.*,bb.cCusPPerson,isnull(bb.cCusPhone,' ') 'cCusPhone',cc.*,0 lngopUserExId,aa.strUserName strAllAcount  from Dl_opUser aa left join Customer bb on aa.cCusCode=bb.cCusCode left join Dl_opSystemConfiguration cc on 1=1  where aa.strLoginName=@username";
            SqlParameter[] paras = new SqlParameter[] { 
            new SqlParameter("@username",username)
             };
            dt = sqlhelper.ExecuteQuery(cmdText, paras, CommandType.Text);
        ...
        #region 加载网页插入二维码登录的sessionID[QrForSessionID]
        /// <summary>
        /// 加载网页插入二维码登录的sessionID[QrForSessionID]
        /// </summary>
        /// <param name="sessionID">sessionID</param>
        /// <returns></returns>
        public bool QrForSessionID(string sessionID)
        {
            string cmdText = "INSERT INTO dbo.QrCodeLogin (sessionID) VALUES (@sessionID)";
            SqlParameter[] paras = new SqlParameter[] { 
